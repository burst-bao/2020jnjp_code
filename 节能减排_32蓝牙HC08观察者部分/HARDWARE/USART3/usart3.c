...
float ad=0;
u8 ad_RX_BUF[4];		//0.1.2用来接收数据，第3位用来判断是哪一个ADC，
...
u8 j=0;	//判断是哪一个AD用的，不能用int，因为会溢出吧
//ad=0;flag=0;i=0;
				...
				else
			{
				if(flag == 1)
				{
					...
					if(i==4)
					{
						...
						//if(1)
						j = ad_RX_BUF[3] -'0';
						OLED_ShowString(0+6*(4+5+3),12*j,"safe!",12);
						OLED_ShowChar(4*6+0,12*j,ad_RX_BUF[0],12);
						OLED_ShowChar(4*6+6,12*j,'.',12);
						OLED_ShowChar(4*6+6*2,12*j,ad_RX_BUF[1],12);
						OLED_ShowChar(4*6+6*3,12*j,ad_RX_BUF[2],12);
						OLED_ShowChar(4*6+6*4,12*j,'V',12);
						OLED_Refresh();
						delay_ms(100);
						i=0;
						j=0;
